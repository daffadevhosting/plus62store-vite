                ...
                generateSimpleCart = function (space) {
                    ...
                    var selectorEngines = {
                        ...
                        currencies = {
                            IDR: {
                                ...
                                decimal: ",", /* Added decimal for IDR */
                                delimiter: "." /* Added delimiter for IDR */
                            },
                        },
                        // default options
                        settings = {
                            ...
                            cartStyle: "div",
                            cartColumns: [
                                { attr: "image", label: "Gambar", view: "image" },
                                { attr: "name", label: "Nama Produk", view: "attr" },
                                { attr: "warna", label: "Warna", view: "warna" },
                                { attr: "ukuran", label: "Ukuran", view: "ukuran" },
                                { attr: "price", label: "Harga", view: "currency" },
                                { attr: "quantity", label: "Jumlah", view: "decrement" },
                                { attr: "quantity", label: "", view: "input" },
                                { attr: "quantity", label: "", view: "increment" },
                                { attr: "berat", label: "Berat", view: "totalberat" }, /* Corrected column for berat */
                                { attr: "total", label: "Subtotal", view: "currency" },
                                { attr: "remove", label: "Hapus", view: "remove" }
                            ],

                            excludeFromCheckout: ["image", "warna", "ukuran", "berat"],
                    ...
                    cartColumnViews = {
                        ...
                        decrement: function (item, column) {
                            return ["<a href='javascript:;' class='" + namespace + "_decrement hidden'>" + tombolMinus + "</a>"];
                        },
                        increment: function (item, column) {
                            return ["<a href='javascript:;' class='" + namespace + "_increment hidden'>" + tombolPlus + "</a>"];
                        },
                        image: function (item, column) {
                            return ["<img src='" + item.get(column.attr) + "'/>"];
                        },
                        name: function (item, column) {
                            return item.get(column.attr) || ""; // Display product name directly
                        },
                        input: function (item, column) {
                            return ["<input type='text' value='" + item.get(column.attr) + " Pcs' class='" + namespace + "_input' disabled/>"];
                        },
                        berat: function (item, column) {
                            return [item.get(column.attr)];
                        },
                        totalberat: function (item, column) {
                            return "<span class='total-berat' data-berat='" + item.get(column.attr) + "'>" + formatBerat(item.get(column.attr)) + "</span>";
                        },
                    ...
                    simpleCart.extend({
                        ...
                        writeCart: function (selector) {
                           ...
                           // Hide specific elements if cart is empty, show cart empty message
                           if (simpleCart.quantity() === 0) {
                                $(".ck-btn,.basket-detail,.harga-total,#total-harga,.right_side,.lisQdaffadev,.chatWhatsapp").hide();
                                $(".keranjang-kosong").show(); // Ensure this is visible if cart is empty
                                container.html(" ").append(cart_kosong); // Display the empty cart message
                                return cart_container; // Exit early if cart is empty
                            } else {
                                $(".ck-btn,.basket-detail,.harga-total,#total-harga,.right_side,.lisQdaffadev,.chatWhatsapp").show();
                                $(".keranjang-kosong").hide(); // Hide if cart is not empty
                                $(".empty-cart").hide(); // Hide the empty cart message if items exist
                            ...
                            cart_container.append(thead_container); // Add the header to the cart container
                    ...
                    simpleCart.Item = function (info) {
                        ...
                        me.set = function (name, value) {
                            if (!isUndefined(name)) {
                                ...
                                if (name.toLowerCase() === "price" || name.toLowerCase() === "quantity" || name.toLowerCase() === "berat") {
                    ...
                    simpleCart.Item._ = simpleCart.Item.prototype = {
                        ...
                        reservedFields: function () {
                            return ["quantity", "id", "item_number", "price", "name", "shipping", "tax", "taxRate", "berat"]; /* Added 'berat' to reserved fields */
                        },
                        ...
                        berat: function (val) {
                            return isUndefined(val) ? parseInt(this.get("berat", true) || 0, 10) : this.set("berat", val); /* Default berat to 0 if undefined */
                        },
                        totalberat: function () {
                        ...
                        checkout: function (opts) {
                            ...
                            simpleCart.each(function (item, x) {
                                var counter = x + 1,
                                    options_list = [],
                                    send;
                                data["item_thumb_" + counter] = item.get("image");
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    // Anda bisa menambahkan logika setelah sukses checkout di sini
                                    console.log("Checkout successful:", data);
                                },
                                error: function(jqXHR, textStatus, errorThrown) {
                                    console.error("Checkout failed:", textStatus, errorThrown);
                                }
                            });
                        },
                    });
                    ...
                    simpleCart.extend({
                        toCurrency: function (number, opts) {
                            ...
                            // Menggunakan fungsi reverse() yang dimodifikasi untuk delimiter
                            var reversedInts = ints.split('').reverse().join('');
                            ints = simpleCart.chunk(reversedInts, 3).join(_opts.delimiter).split('').reverse().join('');
                        ...
                        simpleCart.bindInputs([
                            {
                                ...
                                callback: function () {
                                    simpleCart.checkout({
                                        url: "YOUR_CHECKOUT_PROCESSING_URL_HERE", // Ganti dengan URL pemrosesan checkout Anda
                                        method: "POST" // Bisa GET atau POST
                                    });
                                },
                            },
                            {
